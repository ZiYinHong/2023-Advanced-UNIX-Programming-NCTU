/*
	another way to write code:
		first store all code_*() function in the array of function pointer (func_addr)
		then when overwrite GOT table later, we can access actual address of each code_*() function (or say pointer to each code_*() function)
		from the func_addr array, then put the address into right GOT table entry.
*/
#include <dlfcn.h>
#include <stdio.h>
#include <sys/types.h>
...
int GOTLen = sizeof(GOT)/sizeof(GOT[0]);
...
int get_index_of_rightFun(int code_num){
...
uid_t init() {
	...
	//store all code_*() function pointer in a array
	void (*func)(void) = NULL; //function (input void, return void) pointer
	void (*func_addr[arrLen])(void); //func_addr is an array which store function pointer
    char func_name[20];
    if(func == NULL) {
		...
		if(handle != NULL) {
			for(int i = 0; i < arrLen; i++){
            	...
            	sprintf(func_name, "code_%d", ndat[i]);
	...
	//calculate GOT start address (first entry's address) & mprotect_start address (need to be page align)
	...
	//get the code func pointer from access func_addr, and assign it to GOT entry (pointer to pointer)
	int index, rightFun_index;
	for(int i = 0; i < GOTLen; i++){
		if((index = get_index_of_rightFun(GOT[i])) >= 0){
			if((rightFun_index = get_index_of_rightFun(index)) >= 0){
				//memcpy((void **)chals_1_start + GOTentry_relative_addr[i], &func, sizeof(&func)); 
				*(void** )(chals_1_start + GOTentry_relative_addr[i]) = func_addr[rightFun_index];	
			}
			else{
